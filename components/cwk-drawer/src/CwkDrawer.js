 ...
 * I might decide later to depend on it directly and do only necessary overrides
 * but it is not clear to me whether this drawer is meant to be consumed/extended like that.
 *
...
export class CwkDrawer extends OverlayMixin(LitElement) {
  ...
  _setupOverlayCtrl() {
    ...
    this._overlayCtrl.transitionHide = async ({ contentNode }) => {
      contentNode.style.transition =
        'transform 0.20s cubic-bezier(0.4, 0.0, 0.2, 1), var(--cwk-background-transition), var(--cwk-fill-transition)';
  ...
  constructor() {
    ...
    this.mediaMatcher = window.matchMedia('(min-width: 600px)');
  ...
  onGestureStart(ev) {
    ...
    this._overlayCtrl.contentNode.style.transition =
      'var(--cwk-background-transition), var(--cwk-fill-transition)';
